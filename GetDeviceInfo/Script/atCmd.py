...
resetToFN = {
	'cmd': "ATZ",
	'respPattern': r"(?P<name>\w+)" }

createNwkCmd = {
	'cmd': "AT$JNNWK=%(channel)02x",
...
simpleDescriptorCmd = {
	'cmd': "AT$SIMPDESC=%(shortAddr)04x,%(endPoint)02x" }

bindReqCmd = {
	'cmd': "AT$BIND=%(action)02x,%(dest)02x,%(srcEp)02x,%(extAddr)016x,%(destEp)02x,%(clusterId)04x" }

remoteAttrReqCmd = {
        'cmd': "AT$ATTRRR=%(shortAddr)04x,%(endPoint)02x,%(clusterId)04x,%(attrValue)04x" }
...
def sendCommandNoResponsePattern(serialport, cmd, **inputs):
	cmdStr = cmd['cmd'] % inputs
	#print inputs
	result = []
	serialport.sendBuffer(cmdStr + COMMAND_SEPARATOR)
