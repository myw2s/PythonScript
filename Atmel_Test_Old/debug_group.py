...
if (joinedDevice):
							...
							if (profileID == ZLL_PROFILE_ID):
								...
								if (numOfOutputCluster != 0):
									for numOfOutputCluster in range(0, numOfOutputCluster):
										...
										if (strArraySimpleDescReq[5 + numOfOutputCluster] == '0004'):
											# Group cluster
											devices[index].addSupportedClusterToList('0004')
	...
	for index in range(0, joinedDevice):
		...
		if (devices[index].isClusterSupported('0004')):
			response = sendCommandNoResponsePattern(serialport, addGroupCmd, shortAddr = int(shortAddr,base=16), endPoint = int(strEpList[numOfEP], base=16), groupId = 1)
