	...
	for testCounts in range(0, TOTAL_TEST_COUNTS):
		...
		if (devices[index].isClusterSupported('0008')):
			...
			if (response.startswith('ERROR')):
				...
				logMsg = '[' + response + ' Level control] --- Device #' + str(index + 1) + '__' + str(devices[index].getMacAddress() + '\n'
		...
		if (devices[index].isClusterSupported('0300')):
			...
			if (response.startswith('ERROR')):
				errorCounts += 1
				response, dummy = response.split('\r\n')
				logMsg = '[' + response + ' Color control] --- Device #' + str(index + 1) + '__' + str(devices[index].getMacAddress() + '\n'
				logFile.write(logMsg)
